...
@testset "Chebyshev1 Orthogonality" begin
    for m in 0:max_n
        for n in 0:max_n
            Tm_sym = chebyshev1(m, x)
            Tn_sym = chebyshev1(n, x)

            f_m = symbolic_to_function(Tm_sym)
            f_n = symbolic_to_function(Tn_sym)
