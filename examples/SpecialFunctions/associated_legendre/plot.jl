...
p = plot(title="Asociated Legendre Polynomials \$ P^{-$L}_$L \\sim P^{$L}_$L \$", legend=:topright)

for m in -L:L
    Pn_sym = associated_legendre(m, L, x)
    fn_expr = Symbolics.build_function(Pn_sym, x)
    Pn_func = eval(fn_expr)
    
    ys = [ Pn_func(xi) for xi in xs ]

    plot!(xs, ys, label="\$ P^{$m}_$L(x) \$", color=colors[mod1(m+L+1, length(colors))])
end
