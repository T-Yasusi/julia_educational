function theta3(z::Complex, tau::Complex; atol=1e-8, rtol=1e-8, Nmax = 1000)
...
#    println(m)
    factor *= exp(pi*im*m^2*tau+2*pi*im*m*z)
    z += m*tau
...
#    println("3. z = ", z, "  tau = ", tau, "  q = ", q)    
    ...
    n = 1
    while n <= Nmax
#        term = q^(n^2) * ( exp(2*im*n*pi*z) + exp(-2*im*n*pi*z) )
        term = q^(n^2) * 2 * cos(2*pi*n*z)
        s_new = s + term
